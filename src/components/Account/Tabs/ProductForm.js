...
const ProductForm = ({
  ...
  const handleImageUpload = async (e, type) => {
    ...
    try {
      const urls = await Promise.all(
        files.map((f) => uploadToFirebase(f, type)),
      );
      ...
      if (type === "image") {
        ...
        setFormData((prev) => ({
          ...prev,
          thumbImage: [...prev.thumbImage, ...urls],
        }));
      }
    } catch (err) {
      addToast(t("error_image_upload_failed"), {
        appearance: "error",
        autoDismiss: true,
      });
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name[currentLanguage]?.trim())
      newErrors.name = "error_name_required";
    if (!formData.shortDescription[currentLanguage]?.trim())
      newErrors.shortDescription = "error_short_description_required";
    if (!formData.fullDescription[currentLanguage]?.trim())
    ...
    <Modal show={show} onHide={onHide} centered size="xl" backdrop="static">
      ...
      <Modal.Body style={{ maxHeight: "70vh", overflowY: "auto" }}>
        <Form>
            ...
            <Col md={12}>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    handleBilingualChange("name", e.target.value)
                  }
            ...
            <Col md={12}>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    handleBilingualChange("shortDescription", e.target.value)
                  }
            ...
            <Col md={12}>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    handleBilingualChange("fullDescription", e.target.value)
                  }
          ...
          <Row className="mb-4">
            ...
            <Col md={4}>
              <Form.Group>
                <Form.Label className="fw-bold">{t("is_service")}</Form.Label>
                <Form.Text
                  className="text-muted d-block mb-1"
                  style={{ fontSize: "0.72rem" }}
                >
                ...
                <div className="single-method">
                  ...
                  />
                  <label htmlFor="is_service">{t("is_service")}</label>
                </div>
              </Form.Group>
            </Col>
      ...
      <Modal.Footer>
        ...
        <Button
          ...
          disabled={isLoading || uploadingImage || uploadingThumb}
        >
          {isLoading ? (
