...
const ProductForm = ({
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        price: {
          en: product.price?.en ?? 0,
          mk: product.price?.mk ?? 0,
        },
  ...
  const validateForm = () => {
    ...
    if (!formData.name.en || !formData.name.mk) {
      newErrors.name = "error_name_required";
    }
    if (!formData.shortDescription.en || !formData.shortDescription.mk) {
      newErrors.shortDescription = "error_short_description_required";
    }
    if (!formData.fullDescription.en || !formData.fullDescription.mk) {
      newErrors.fullDescription = "error_full_description_required";
    }
    if (!formData.sku) {
      newErrors.sku = "error_sku_required";
    }
    if (!formData.slug) {
      newErrors.slug = "error_slug_required";
    }
    if (Number(formData.price.en) <= 0 || Number(formData.price.mk) <= 0) {
      newErrors.price = "error_price_required";
    }
    if (Number(formData.stock) < 0) {
      newErrors.stock = "error_stock_negative";
    }
    if (Number(formData.discount) < 0 || Number(formData.discount) > 100) {
      newErrors.discount = "error_discount_range";
    }
    if (formData.category.length === 0) {
      newErrors.category = "error_category_required";
    }
      ...
      <Row className="mb-4">
        ...
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control
              type="text"
              value={formData.name.mk}
              onChange={(e) => handleInputChange("name", e.target.value, "mk")}
              isInvalid={!!errors.name}
              placeholder={t("enter_macedonian_name")}
            />
          </Form.Group>
        </Col>
      </Row>
        ...
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                handleInputChange("shortDescription", e.target.value, "en")
              }
              isInvalid={!!errors.shortDescription}
              placeholder={t("enter_short_description_english")}
            />
          </Form.Group>
        </Col>
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control
              as="textarea"
              rows={3}
              value={formData.shortDescription.mk}
              onChange={(e) =>
                handleInputChange("shortDescription", e.target.value, "mk")
              }
              isInvalid={!!errors.shortDescription}
              placeholder={t("enter_short_description_macedonian")}
            />
          </Form.Group>
        </Col>
      </Row>
        ...
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                handleInputChange("fullDescription", e.target.value, "en")
              }
              isInvalid={!!errors.fullDescription}
              placeholder={t("enter_full_description_english")}
            />
          </Form.Group>
        </Col>
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control
              as="textarea"
              rows={4}
              value={formData.fullDescription.mk}
              onChange={(e) =>
                handleInputChange("fullDescription", e.target.value, "mk")
              }
              isInvalid={!!errors.fullDescription}
              placeholder={t("enter_full_description_macedonian")}
            />
          </Form.Group>
        </Col>
      </Row>
          ...
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                handleInputChange(
                  "rating",
                  Number.isNaN(parseInt(e.target.value, 10)) ? 5 : parseInt(e.target.value, 10)
                )
              }
              placeholder="5"
            />
